...
struct Bound
{
    ...
    KITTEN_FUNC_DECL Bound() : min(vec3(INFINITY)), max(vec3(-INFINITY))
    {
    }

    KITTEN_FUNC_DECL Bound(vec<dim, Real, defaultp> center)
        : min(center), max(center)
    {
    }
    KITTEN_FUNC_DECL Bound(vec<dim, Real, defaultp> min,
                           vec<dim, Real, defaultp> max)
        : min(min), max(max)
    {
    }
    ...
    KITTEN_FUNC_DECL inline bool contains(
        const vec<dim, Real, defaultp>& point) const
    {
        return all(lessThanEqual(min, point)) &&
               all(greaterThanEqual(max, point));
    }

    KITTEN_FUNC_DECL inline bool contains(const Bound<dim, Real>& b) const
    {
        return all(lessThanEqual(min, b.min)) &&
               all(greaterThanEqual(max, b.max));
    }

    KITTEN_FUNC_DECL inline bool intersects(const Bound<dim, Real>& b) const
    {
        return !(any(lessThanEqual(max, b.min)) ||
                 any(greaterThanEqual(min, b.max)));
    }
    ...
    KITTEN_FUNC_DECL inline vec<dim, Real, defaultp> normCoord(
        const vec<dim, Real, defaultp>& pos) const
    {
        return (pos - min) / (max - min);
    }

    KITTEN_FUNC_DECL inline vec<dim, Real, defaultp> interp(
        const vec<dim, Real, defaultp>& coord) const
    {
...
struct Bound<1, Real>
{
    ...
    KITTEN_FUNC_DECL Bound() : min(INFINITY), max(-INFINITY)
    {
    }
