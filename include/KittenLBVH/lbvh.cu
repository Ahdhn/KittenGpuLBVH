...
template <typename T>
__global__ void mortonKernel(Bound<3, T>* aabbs,
                             uint32_t*    codes,
                             int*         ids,
                             Bound<3, T>  wholeAABB,
                             int          size)
                                       ...
                                       uint32_t        idx)
{
    ...
    const int      lDelta =
        commonUpperBits(selfCode, mergeIdx(mortonCodes[idx - 1], idx - 1));
    const int rDelta =
        commonUpperBits(selfCode, mergeIdx(mortonCodes[idx + 1], idx + 1));
    ...
    while (t > 0) {
        i = idx + (l + t) * d;
        if (0 <= i && i < numObjs)
            if (commonUpperBits(selfCode, mergeIdx(mortonCodes[i], i)) >
                minDelta)
                l += t;
        t >>= 1;
    }
    ...
    if (d < 0)
        thrust::swap(idx, jdx);  // Make sure that idx < jdx
                                     ...
                                     const uint32_t  last)
{
    ...
    do {
        ...
        if (middle < last)
            if (commonUpperBits(firstCode,
                                mergeIdx(mortonCodes[middle], middle)) >
                deltaNode)
                split = middle;
    } while (stride > 1);
...
__global__ void lbvhBuildLeafKernel(LBVH::node*      leafNodes,
                                    Bound<3, float>* leafAABBs,
                                    Bound<3, float>* objAABBs,
                                    int const*       objIDs,
                                    int              numObjs)
{
                                        ...
                                        int             numObjs)
{
    ...
    // Check if there are leaf nodes, which are indexed behind the (numObj - 1)
    // internal nodes
    ivec2 children((range.x == gamma) ? gamma + numObjs - 1 : gamma,
                               ...
                               int              numObjs)
{
    ...
    if (tid <
        numObjs -
            1)  // Reset internal flags because we need to refit after this
        flags[tid] = 0;
}
...
// We do 128 wide blocks which gets us 8KB of shared memory per block on
// compute 8.6. Realistically, though, we are limited by the number of registers
// so we can overallocate shared memory.
...
__global__ void lbvhQueryKernel(ivec2*                 res,
                                int*                   resCounter,
                                int                    maxRes,
                                LBVH::node const*      nodes,
                                Bound<3, float> const* aabbs,
                                LBVH::node const*      queryNodes,
                                Bound<3, float> const* queryAabbs,
                                const int              numQueries)
{
    ...
    __shared__ int
        sharedCounter;  // How many results are cached in shared memory
    ...
    while (true) {
        ...
        if (active)
            while (stackPtr != stack) {
                ...
                if (node.isLeaf()) {
                    if (IGNORE_SELF)
                        if (nodeIdx <= tid + numQueries - 1)
                            continue;
                    // Add to shared memory
                    int sIdx = atomicAdd(&sharedCounter, 1);
                    if (sIdx >= MAX_RES_PER_BLOCK) {
                        // We cannot sync here so we push the node back on the
                        // stack and wait
                        *(stackPtr++) = nodeIdx;
                        break;
                    }
                    ...
                    if (IGNORE_SELF)
                        if (glm::max(nodeIdx, node.fence) <= tid)
                            continue;
...
// We are primarily limited by the number of registers, so we always call the
// kernel with just enough stack space. This gives another ~15% performance
// boost for queries.
template <bool IGNORE_SELF>
void launchQueryKernel(ivec2*                 res,
                       int*                   resCounter,
                       int                    maxRes,
                       LBVH::node const*      nodes,
                       Bound<3, float> const* aabbs,
...
#define DISPATCH_QUERY(N)                                    \
    case N:                                                  \
        lbvhQueryKernel<IGNORE_SELF, N>                      \
            <<<(numQueries + 127) / 128, 128>>>(res,         \
                                                resCounter,  \
                                                maxRes,      \
                                                nodes,       \
                                                aabbs,       \
...
void LBVH::compute(aabb* devicePtr, size_t size)
{
    ...
    const unsigned int numInternalNodes =
        numObjs - 1;  // Total number of internal nodes
    ...
    // Compute the bounding box for the whole scene so we can assign morton
    // codes
    aabb wholeAABB;
    ...
    thrust::stable_sort_by_key(
        d_morton.begin(), d_morton.end(), d_objIDs.begin());
    ...
    LBVHKernels::
        lbvhBuildInternalKernel<<<(numInternalNodes + 255) / 256, 256>>>(
            thrust::raw_pointer_cast(d_nodes.data()),
            thrust::raw_pointer_cast(d_flags.data()),
            thrust::raw_pointer_cast(d_morton.data()),
            numObjs);
    ...
    int                                           idx)
{
    ...
    // Check if they match
    if (length2(left.min - aabb.min) > 1e-7f ||
        length2(left.max - aabb.max) > 1e-7f) {
...
ivec2 lbvhCheckIndexMerge(thrust::host_vector<LBVH::node>& nodes,
                          int                              idx,
                          int                              numObjs)
{
...
void LBVH::bvhSelfCheck() const
{
    ...
    // Check sizes
    if (nodes.size() != numObjs * 2 - 1 || aabbs.size() != numObjs * 2 - 1 ||
        morton.size() != numObjs)
    ...
    for (size_t i = 0; i < numObjs - 1; i++) {
        ...
        if (nodes[node.leftIdx].parentIdx != i ||
            nodes[node.rightIdx].parentIdx != i)
    ...
    if (numVisited != numObjs * 2 - 1)
        printf(
            "Error: Not all nodes are accessible from the root. Only %d/%d "
            "nodes are found.\n",
            numVisited,
            2 * numObjs - 1);
